...
import { P2PPairingRequest, StorageKey, P2PPairingResponse, Regions } from '@airgap/beacon-types';
...
export class P2PCommunicationClient extends CommunicationClient {
    ...
    iconUrl;
    appUrl;
    client = new ExposedPromise();
    initialEvent;
    initialListener;
    selectedRegion;
    ENABLED_RELAY_SERVERS;
    relayServer;
    ...
    async getPairingRequestInfo() {
        const info = new P2PPairingRequest(await generateGUID(), this.name, await this.getPublicKey(), BEACON_VERSION, (await this.getRelayServer()).server);
        if (this.iconUrl) {
    ...
    async getPairingResponseInfo(request) {
        const info = new P2PPairingResponse(request.id, this.name, await this.getPublicKey(), request.version, (await this.getRelayServer()).server);
        if (this.iconUrl) {
            info.icon = this.iconUrl;
        }
        if (this.appUrl) {
     ...
     */
    async findBestRegionAndGetServer() {
        ...
        const timeoutPromise = new ExposedPromise();
        keys.forEach((key) => {
    ...
    async tryJoinRooms(roomId, retry = 1) {
        try {
    ...
    async start() {
        ...
        this.initialListener = async (event) => {
        ...
        logger.log('start', `login ${loginString}, ${await this.getPublicKeyHash()} on ${relayServer.server}`);
        const loginRawDigest = hash(encode(loginString), 32);
        const secretKey = this.keyPair.secretKey ?? this.keyPair.privateKey;
    ...
    async sendMessage(message, peer) {
        ...
        const recipientHash = await getHexHash(Buffer.from(peer.publicKey, 'hex'));
    ...
    async updatePeerRoom(sender, roomId) {
    ...
    async deleteRoomIdFromRooms(roomId) {
        const roomIds = await this.storage.get(StorageKey.MATRIX_PEER_ROOM_IDS);
    ...
    async waitForJoin(roomId, retry = 0) {
    ...
    async sendPairingResponse(pairingRequest) {
        ...
        const encryptedMessage = await this.encryptMessageAsymmetric(pairingRequest.publicKey, message);
    ...
    async updateRelayServer(sender) {
    ...
    async isChannelOpenMessage(content) {
        return content.message.content.startsWith(`@channel-open:@${await getHexHash(Buffer.from(await this.getPublicKey(), 'hex'))}`);
    }
    async isSender(event, senderPublicKey) {
        return event.content.message.sender.startsWith(`@${await getHexHash(Buffer.from(senderPublicKey, 'hex'))}`);
    }
    ...
    async getRelevantJoinedRoom(recipient) {
