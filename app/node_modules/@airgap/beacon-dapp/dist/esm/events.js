 ...
 */
const showSentToast = async (data) => {
 ...
 * Show a
 */
const showInvalidActiveAccountState = async () => {
    await openAlert({
        title: 'Invalid state',
 ...
 */
const showErrorToast = async (response, buttons) => {
 ...
 */
const showExtensionConnectedAlert = async () => {
    await closeAlerts();
};
/**
 * Show a "channel closed" alert for 1.5 seconds
 */
const showChannelClosedAlert = async () => {
    await openAlert({
...
const showInternalErrorAlert = async (data) => {
 ...
 */
const showPairAlert = async (data) => {
...
const showProofOfEventChallengeSuccessAlert = async (data) => {
    const { output } = data;
    await openToast({
        ...
        actions: [
            {
                ...
                actionLogo: 'external',
                actionCallback: async () => {
                    const link = await blockExplorer.getTransactionLink(output.transactionHash, account.network);
        ...
        actions: [
            {
                ...
                actionLogo: 'external',
                actionCallback: async () => {
                    const link = await blockExplorer.getTransactionLink(output.transactionHash, network);
                    window.open(link, '_blank', 'noopener');
                    await closeToast();
                }
            }
...
export class BeaconEventHandler {
     ...
     */
    async emit(event, data, eventCallback) {
        const listeners = this.callbackMap[event];
