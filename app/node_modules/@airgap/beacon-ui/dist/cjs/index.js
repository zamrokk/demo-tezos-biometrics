...
const isBrowser = win => testUserAgent(win, /(?:Mozilla)(?=.*\b(?:Gecko|AppleWebKit))(?=.*\b(?:Chrome|Safari|Firefox|Opera|Edge|Brave|DuckDuckGo)).*/i);
const isPrivacyBrowser = win => testUserAgent(win, /Mobile DuckDuckGo/i);
const isIOS = win => isPrivacyBrowser(win) || testUserAgent(win, /iPhone|iPod|Mobile DuckDuckGo/i) || isIpad(win);
const isAndroid = win => !isPrivacyBrowser(win) && testUserAgent(win, /android|sink/i);
const isTwBrowser = win => win && win.ethereum?.isTrust == true;
...
const openAlert = async config => {
  ...
  if (!isOpen$1()) {
    let availableExtensions = await beaconTransportPostmessage.PostMessageTransport.getAvailableExtensions();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const extensionsUpdatedFn = async event => {
      if (event.data === 'extensionsUpdated') {
        availableExtensions = await beaconTransportPostmessage.PostMessageTransport.getAvailableExtensions();
    ...
    const createWalletList = () => {
      ...
      .filter(wallet => !availableExtensions.some(extWallet => wallet.name === extWallet.name)).map(wallet => {
        ...
        const link = wallet.links[config.pairingPayload?.networkType ?? beaconTypes.NetworkType.MAINNET];
        return {
          id: wallet.key,
          key: wallet.key,
          name: wallet.shortName,
    ...
    const handleNewTab = async (config, wallet) => {
      if (!wallet) {
        return;
      }
      if (config.pairingPayload) {
    ...
    const handleClickWallet = async id => {
      ...
      if (wallet?.key) {
        analytics()?.track('click', 'ui', 'opened wallet', {
          key: wallet.key
        });
        localStorage.setItem(beaconTypes.StorageKey.LAST_SELECTED_WALLET, wallet.key);
      }
      if (wallet?.types.includes('web') && !(wallet?.types.includes('extension') || wallet?.types.includes('desktop') || wallet?.types.includes('ios'))) {
      ...
      if (wallet && wallet.supportedInteractionStandards?.includes('wallet_connect')) {
        ...
        if (config.pairingPayload) {
          const serializer = new beaconCore.Serializer();
          const code = await serializer.serialize(await p2pPayload);
          const link = getTzip10Link(isIOS(window) && wallet.deepLink ? wallet.deepLink : isAndroid(window) ? 'tezos://' : wallet.links[OSLink.IOS], code);
    ...
    const handleClickConnectExtension = async () => {
      analytics()?.track('click', 'ui', 'open extension', {
    ...
    const handleClickInstallExtension = async () => {
      analytics()?.track('click', 'ui', 'install extension', {
        key: currentWallet()?.key
      });
      setShowMoreContent(false);
    ...
    const handleClickOpenDesktopApp = async () => {
      setShowMoreContent(false);
      analytics()?.track('click', 'ui', 'open desktop', {
        key: currentWallet()?.key
      });
      if (p2pPayload) {
        const serializer = new beaconCore.Serializer();
    ...
    const handleClickDownloadDesktopApp = async () => {
      analytics()?.track('click', 'ui', 'download desktop', {
        key: currentWallet()?.key
      });
      setShowMoreContent(false);
      window.open(currentWallet()?.links[OSLink.DESKTOP] || '', '_blank', 'noopener');
    };
    const hasExtension = () => availableExtensions.map(extension => extension.id).includes(currentWallet()?.firefoxId || '') || availableExtensions.map(extension => extension.id).includes(currentWallet()?.id || '');
    const colorMode = getColorMode();
    dispose$1 = web.render(() => (() => {
      const _el$ = _tmpl$$1();
      web.className(_el$, `theme__${colorMode}`);
      web.insert(_el$, web.createComponent(AlertResize, {}), null);
      web.insert(_el$, (() => {
            ...
            return (() => {
              ...
              web.insert(_el$2, (() => {
                ...
                return () => _c$3() && (() => {
                  const _el$6 = _tmpl$$1();
                  web.insert(_el$6, (() => {
                    const _c$5 = web.memo(() => !!(!isMobile() && isOnline && currentWallet()?.types.includes('web')));
                    return () => _c$5() && web.createComponent(Info, {
                      border: true,
                      title: 'Open wallet in a new tab',
                      get description() {
                        return `Please connect below to use ${currentWallet()?.name}`;
                      },
                      buttons: [{
                        label: 'Connect now',
                        type: 'primary',
                  ...
                  web.insert(_el$6, (() => {
                    const _c$6 = web.memo(() => !!(!isMobile() && currentWallet()?.types.includes('extension')));
                    return () => _c$6() && web.createComponent(Info, {
                      border: true,
                      get title() {
                        return web.memo(() => !!hasExtension())() ? `Use Browser Extension` : `Install ${currentWallet()?.name} Wallet`;
                      },
                      get description() {
                        return web.memo(() => !!hasExtension())() ? `Please connect below to use your ${currentWallet()?.name} Wallet browser extension.` : `To connect your ${currentWallet()?.name} Wallet, install the browser extension.`;
                      },
                      get buttons() {
                        return hasExtension() ? [{
                          label: 'Connect now',
                  ...
                  web.insert(_el$6, (() => {
                    const _c$7 = web.memo(() => !!(!isMobile() && currentWallet()?.types.includes('desktop')));
                    return () => _c$7() && web.createComponent(Info, {
                      border: true,
                  ...
                  web.insert(_el$6, (() => {
                    const _c$8 = web.memo(() => !!(!isMobile() && codeQR().length > 0 && currentWallet()?.types.includes('ios') && currentWallet()?.types.length > 1));
                    return () => _c$8() && web.createComponent(QR, {
                      get isWalletConnect() {
                        return currentWallet()?.supportedInteractionStandards?.includes('wallet_connect') || false;
                      },
                      isMobile: false,
                      get walletName() {
                        return currentWallet()?.name || 'AirGap';
                      },
                      get code() {
                        return codeQR();
                      },
                  ...
                  web.insert(_el$6, (() => {
                    const _c$9 = web.memo(() => !!(!isMobile() && codeQR().length > 0 && currentWallet()?.types.includes('ios') && currentWallet()?.types.length <= 1));
                  ...
                  web.insert(_el$6, (() => {
                    const _c$10 = web.memo(() => !!(isMobile() && codeQR().length > 0));
                    return () => _c$10() && web.createComponent(QR, {
                      get isWalletConnect() {
                        return currentWallet()?.supportedInteractionStandards?.includes('wallet_connect') || false;
                      },
                      isMobile: true,
...
const Toast = props => {
  ...
  return (() => {
    const _el$4 = _tmpl$4(),
      _el$5 = _el$4.firstChild,
      _el$6 = _el$5.firstChild;
    _el$4.$$dblclick = onClickHandler;
    _el$4.$$click = onClickHandler;
...
web.delegateEvents(["mousedown", "click", "dblclick"]);
...
const openToast = async config => {
  ...
  const id = await beaconUtils.generateGUID();
  setRenderLast(id);
  await closeToast();
